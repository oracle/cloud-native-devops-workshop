...
There are a total of 27 target variables that need to be mapped to successfully execute the EBS PROCESS_ORDER API call.  The next 4 sections will guide you through the process of mapping all these attributes.  Mapping many fields can be a common task while working with integrations.  ICS has the capability to do very complex mappings if necessary.
...
**2.6.9** Drag and Drop the function `current-date` into the area labeled `-- Drag and Drop or Type here...`
