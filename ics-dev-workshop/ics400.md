...
### **2.5**	Mapping the **RESTHeader** and **InputParameters** Target Variables for the *createEBSOrder* API invocation
...
### **2.6** Mapping the **P\_HEADER\_REC** Target Variables for the *createEBSOrder* API invocation
...
**2.6.2** Enter the target attribute name `CUST_PO_NUMBER` in the search field, then click on the search *magnifying glass*.  The `CUST_PO_NUMBER` field will be display on top of the target variable list.
...
**2.6.3** We just need a number in the `CUST_PO_NUMBER` target field, so, even though it's not specificall for a PO#, map the source payload `ItemID` over as shown in the following screenshot to assign a number for that field.

![](images/400/image043.png)

**2.6.4** Next, we want to set the current date to the `ORDERED_DATE` header target variable.  Enter the target attribute name `ORDERED_DATE` in the search field, then click on the search *magnifying glass*.

**2.6.5** Click on the `ORDERED_DATE` header target variable to open up the advanced mapping editor.

![](images/400/image044.png)

**2.6.6** Scroll down and select `Mapping Components` on the bottom left of the *Source* side.

**2.6.7** Click and expand *Functions*, then click and expand *Date* functions.

**2.6.8** Drag and Drop the function `current-date` into the area labeled `-- Drag and Drop or Type here...`

![](images/400/image045.png)

**2.6.9** Select the *Save* and *Close* buttons

![](images/400/image046.png)

**2.6.10** Search for and set the following `P_HEADER_REC` values:
...
**2.6.11** We have mapped a lot of values and it's difficult to see what has been accomplished on the Target side because we were using the *Search* feature.  In order to make it easy to see what has been mapped, we will apply a *Filter* to the target variable side.

**2.6.12** Click on the *Filter* button on the *Target* side of the mapping editor.

**2.6.13** In the *Filter* dialog box in the *Is it Mapped?* section, select the radio button `Mapped`, then select the *Apply* button.

![](images/400/image047.png)

**2.6.14** You will now have a good view of all the currently mapped fields in the *RESTHeader*, *InputParameters*, and *P_HEADER_REC* target variable sections.

**2.6.15** Compare your work to the screenshot below:

![](images/400/image048.png)

**2.6.16** It is a good practice to occasionally save your work during a long mapping exercise.  Click on the *Save* button in the upper right of the mapper to save your work up to this point.

### **2.7** Mapping the *P\_LINE\_TBL* Target Variables for the *createEBSOrder* API invocation
...
<table border=2, border-width=2>
  ...
  <tr><td>Mapping Components -> Functions -> Date -> current-date()</td><td>PRICING_DATE</td></tr>
...
### **2.8:**	Testing the Updated Mapping
