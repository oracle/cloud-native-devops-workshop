...
**2.5.15** Next we need to map the EBS payload `P_HEADER_REC` values.  There are hundreds of attributes in the `P_HEADER_REC` so we will leverage the *Search* field to find the fields we need to map.

**2.5.16** Enter the target attribute name `CUST_PO_NUMBER` in the search field, then click on the search *magnifying glass*.

![](images/400/image042.png)

**2.5.17** We just need a number in the `CUST_PO_NUMBER` target field, so just map the source payload `ItemID` over as shown in the following screenshot

![](images/400/image043.png)

**2.5.18** Next, we want to set the current date to the `ORDERED_DATE` header target variable.  Enter the target attribute name `ORDERED_DATE` in the search field, then click on the search *magnifying glass*.

**2.5.19** Click on the `ORDERED_DATE` header target variable to open up the advanced mapping editor.

![](images/400/image044.png)

**2.5.19** Scroll down and select `Mapping Components` on the bottom left of the *Source* side.

**2.5.20** Click and expand *Functions*, then click and expand *Date* functions.

**2.5.21** Drag and Drop the function `current-date` into the area labeled `-- Drag and Drop or Type here...`

![](images/400/image045.png)

**2.5.22** Select the *Save* and *Close* buttons

![](images/400/image046.png)

**2.5.23** Search and set the following `P_HEADER_REC` values:
...
<table border=2, border-width=2>
  ...
  <tr><td>constant: USD (no quotes)</td><td>TRANSACTIONAL_CURR_CODE</td></tr>
  <tr><td>constant: CREATE (no quotes)</td><td>OPERATION</td></tr>
</table>

**2.5.24** We have mapped a lot of values and it's difficult to see what has been accomplished on the Target side because we were using the *Search* feature.  In order to make it easy to see what has been mapped, we will apply a *Filter* to the target variable side.

**2.5.25** Click on the *Filter* button on the *Target* side of the mapping editor.

**2.5.26** In the *Filter* dialog box in the *Is it Mapped?* section, select the radio button `Mapped`, then select the *Apply* button.

![](images/400/image047.png)

**2.5.27** You will now have a good view of all the currently mapped fields in the *RESTHeader*, *InputParameters*, and *P_HEADER_REC* target variable sections.
